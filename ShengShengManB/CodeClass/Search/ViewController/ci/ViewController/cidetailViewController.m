...
@interface cidetailViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
...
//创建一个textfield中的text
@property (nonatomic, strong)NSMutableArray *textFarray;
@property (nonatomic, strong)NSMutableArray *guoluArray;
@end

@implementation cidetailViewController
- (NSMutableArray *)guoluArray
{
    if (!_guoluArray) {
        self.guoluArray = [NSMutableArray new];
    }
    return _guoluArray;
}
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i < self.newarr.count; i++) {
        if ([self.newarr[i][@"content"] length] != 0) {
            [self.guoluArray addObject:self.newarr[i]];
        }
    }
...
- (void)save
{
    if ([UserInfoManager isLoading]) {
        ...
        for (int i = 0; i < self.guoluArray.count; i++) {
        ...
        NSDictionary *editdic = @{@"token":[UserInfoManager getUserInfo].token,@"type":@"ci",@"head":self.headid,@"rhythm":self.rhythmid,@"yunyun":self.yunid,@"body":str,@"notes":bottomstr,@"images":imgstr};
...
- (void)jianchaMethod
{
    ...
    if (self.textFieldArray.count == self.guoluArray.count) {
...
- (void)faburequest
{
    ...
    [NetWorkManager requestForPostWithUrl:bbsaddURL parameter:@{@"token":[UserInfoManager getUserInfo].token,@"poetry":self.headid,@"rhythm":self.rhythmid,@"yunyun":self.yunid,@"type":@"ci",@"subject":contents,@"images":str,@"poetry_notes":poetrynoteStr} success:^(id reponseObject) {
            [MBProgressHUD showError:reponseObject[@"msg"]];
        if ([reponseObject[@"code"] integerValue] == 1) {
            [self.navigationController popViewControllerAnimated:YES];
        }
