...
@interface qudetailViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
- (void)save
{
    ...
    if (self.bottomView.textView.text.length == 0) {
        ...
        bottomstr = self.bottomView.textView.text;
    }
    NSString *imgstr;
    ...
    NSDictionary *editdic = @{@"token":[UserInfoManager getUserInfo].token,@"type":@"qu",@"head":self.headid,@"rhythm":self.rhythmid,@"yunyun":self.yunid,@"body":str,@"notes":bottomstr,@"images":imgstr};
...
- (void)faburequest
{
    ...
    if (self.edittitle.length != 0) {
        contents = [NSString stringWithFormat:@"【%@·%@】%@\n%@/%@\n\n%@",self.edittitle,self.name,self.topView.titleLab.text,self.topView.timeLab.text,self.topView.AuthorLab.text,textfstr];
    } else {
        contents = [NSString stringWithFormat:@"【%@·%@】%@\n%@/%@\n\n%@",self.titlestr,self.name,self.topView.titleLab.text,self.topView.timeLab.text,self.topView.AuthorLab.text,textfstr];
    }
    ...
    [NetWorkManager requestForPostWithUrl:bbsaddURL parameter:@{@"token":[UserInfoManager getUserInfo].token,@"poetry":self.headid,@"rhythm":self.rhythmid,@"yunyun":self.yunid,@"type":@"qu",@"subject":contents,@"images":str,@"poetry_notes":poetrynoteStr} success:^(id reponseObject) {
          [MBProgressHUD showError:reponseObject[@"msg"]];
        if ([reponseObject[@"code"] integerValue] == 1) {
            [self.navigationController popViewControllerAnimated:YES];
        }
...
- (void)rightButtonAction:(UIButton *)sender
{
    _btn.hidden = NO;
    [UIView animateWithDuration:2.0 animations:^{
        NSLog(@"%@",self.bottomView.textView.text);
        self.bottomView.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, 250);
        [self.bottomView removeFromSuperview];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *str = self.newarr[indexPath.row][@"content"];
    if (str.length == 0) {
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    if (cell.textField.text.length != cell.fuhaolab.text.length + 1 && cell.textField.enabled == YES) {
        ...
        for (NSString *newstr in newarr) {
            ...
            NSString *fontstr= [self compelestr11:newstr];
...
- (NSString *)compelestr11:(NSString *)str
{
